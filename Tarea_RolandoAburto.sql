...
BEGIN TRY
	DECLARE @NOMBRESITO VARCHAR(30)
	DECLARE @PAIS VARCHAR(30)
	DECLARE @MONTO INT
	DECLARE NEE CURSOR
	FOR SELECT PASAJERO.NOMBRES, PAIS.PAIS, PAGO.MONTO
		FROM PAIS JOIN PASAJERO
		ON PAIS.IDPAIS = PASAJERO.IDPAIS
		JOIN PAGO
		ON PAGO.IDPASAJERO = PASAJERO.IDPASAJERO
		WHERE PASAJERO.NOMBRES LIKE '%A' AND PAGO.MONTO BETWEEN 500 AND 1500

		OPEN NEE

		FETCH NEE INTO @NOMBRESITO, @PAIS, @MONTO
		WHILE @@FETCH_STATUS = 0
		BEGIN
			PRINT 'NOMBRE: '+@NOMBRESITO
			PRINT 'PAIS: '+@PAIS
			PRINT 'MONTO: '+CAST(@MONTO AS VARCHAR(50))
			FETCH NEE INTO @NOMBRESITO, @PAIS, @MONTO
		END
		CLOSE NEE
		DEALLOCATE NEE
